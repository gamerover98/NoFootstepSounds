...
public class FlatSoundPacketAdapter extends SoundPacketAdapter {
    ...
    public FlatSoundPacketAdapter(@NotNull Plugin plugin, @NotNull ServerVersion currentVersion) {
        ...
        } else if (ServerVersion.is1_18(currentVersion)) {
            soundChecker = new it.gamerover.nfps.support.v1_18.SoundChecker();
        } else if (ServerVersion.is1_19(currentVersion)) {
            soundChecker = new it.gamerover.nfps.support.v1_19.SoundChecker();
        } else if (ServerVersion.is1_20(currentVersion)) {
            soundChecker = new it.gamerover.nfps.support.v1_20.SoundChecker();
        } else {
    ...
    private static class FutureFlatSoundChecker implements ISoundChecker<Sound> {
         ...
         */
        private final ISoundChecker<Sound> latestSupportedChecker = new it.gamerover.nfps.support.v1_18.SoundChecker();
